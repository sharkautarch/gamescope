...
void CVULKANCMDBUFFER_TARGET_ATTR __attribute__((noinline)) CVulkanCmdBuffer::clearBoundTexturesAboveSlot(uint16_t slot) {
	...
	static constexpr uint16_t b = 0b1111'1111'1111'1111;
	static constexpr uint16_t c = 0b1111'1111'1111'1110;
	static constexpr uint16_t B = (uint16_t)~b&(uint16_t)~c;
	static constexpr uint16_t C = b&c;
	//																									\/ also equivalent to & ~((uint16_t)(posToClearTo-2) ^ 0b1111'1111'1111'1111 ) & ~((uint16_t)(posToClearTo-2) ^ 0b1111'1111'1111'1110 ) but with less instructions generated by the compiler
	//						/																			\/ equivalent to & (posToClearTo-2 < 16) -- using the below so that only one conditional jmp insn is generated for this if condition
	if (slotAboveSlot > 16 || oneMinusEndIndex > 16) {
		__builtin_unreachable();
	}
	if (	(oneMinusEndIndex > slotAboveSlot) & (oneMinusEndIndex | B) & (~oneMinusEndIndex|C) ) {
		static_assert( ( ( (uint16_t)(9-2) | B) & (~((uint16_t)(9-2))|C) ) != 0 );
		// let a=posToClearTo2, b=0b1111'1111'1111'1111, c=0b1111'1111'1111'1110
		//~(a^b) & ~(a^c), let A=(a^b), B=(a^c)
		//~A & ~B
		//demorgan: ~A & ~B = ~(A|B)
		
		//~((a^b)|(a^c))
		// (a&~b) | (~a&b) = a ^ b 
		//~((a&~b) | (~a&b) | (a&~c) | (~a&c)) -> demorgan
		//~(a&~b) & ~(~a&b) & ~(a&~c) & ~(~a&c) -> demorgan
		//(~a|b) & (a|~b) & (~a|c) & (a&~c) -> distributive
		//a|(~b&~c) & ~a|(b&c)
		// let B=(~b&~c), let C=(b&c)
		//(a|B) & (~a|C) = a ? b : c
		// = a ? (~b&~c) : (b&c) = posToClearTo2 ? 0b1111'1111'1111'1111 : 0b1111'1111'1111'1110
